export default async function handler(req,res){
  ...
  try{
    ...
    const response=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.GROQ_API_KEY}`},body:JSON.stringify({model,messages:[{role:'system',content:'You are a precise communication classification engine. Output JSON only.'},{role:'user',content:prompt}],temperature:0,response_format:{type:'json_object'},reasoning_effort:'low',include_reasoning:false})});
