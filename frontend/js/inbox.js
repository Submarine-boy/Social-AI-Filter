...
function showDetail(id){
  ...
  detail.innerHTML=`<div class="detail-head"><div><h3>${m.sender}</h3><p class="detail-meta">${m.platform} · ${m.category} · ${m.time}</p></div><strong class="score">${m.priority}/100</strong></div><div class="detail-message">${m.text}</div><div class="analysis-grid"><div class="analysis-item"><small>Priority</small><strong>${m.priority}/100</strong></div><div class="analysis-item"><small>Relevance</small><strong>${m.relevance}/100</strong></div><div class="analysis-item"><small>Needs reply</small><strong>${m.needsReply?'Yes':'No'}</strong></div><div class="analysis-item"><small>Category</small><strong>${m.category}</strong></div></div><p class="detail-reason"><strong>Why it matters:</strong> ${m.reason}</p><div class="detail-actions"><button class="primary" data-action="important">Mark Important</button><button data-action="not-important">Mark Not Important</button><button data-action="spam">Mark as Spam</button><button data-action="similar">Prioritize Similar Messages</button></div>`;
